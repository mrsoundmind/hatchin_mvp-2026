...
// Simulate the new getProjectMemory logic
        ...
        if (error.message.includes('Ambiguous conversation ID')) {
          ...
          if (startsWithPrefix) {
            ...
            try {
              ...
              console.warn(`⚠️ Skipping ambiguous conversationId: ${conversationId}`, retryError.message);
              continue;
            }
          } else {
          ...
          console.warn(`⚠️ Skipping invalid conversationId: ${conversationId}`, error.message);
          continue;
        }
      }
...
const testMemories = new Map<string, any[]>([
  ...
  ['team-saas-startup-design', [{ id: '2', content: 'Team memory 1', importance: 7 }]], // 4 parts - ambiguous
  ['agent-saas-startup-pm', [{ id: '3', content: 'Agent memory 1', importance: 6 }]], // 4 parts - ambiguous
  
  // Different project (should NOT leak)
  ['project-other-project', [{ id: '4', content: 'Other project memory', importance: 9 }]],
  ['team-other-project-design', [{ id: '5', content: 'Other team memory', importance: 8 }]], // 4 parts - ambiguous
  ...
  // Unambiguous team/agent IDs (3 parts) - should match when projectId matches
  ['team-saas-design', [{ id: '9', content: 'Unambiguous team memory', importance: 7 }]], // 3 parts
  ['agent-saas-pm', [{ id: '10', content: 'Unambiguous agent memory', importance: 6 }]], // 3 parts
...
// Should match: project-saas-startup, team-saas-startup-design (with knownProjectId), agent-saas-startup-pm (with knownProjectId)
...
// Should only match unambiguous 3-part IDs: team-saas-design, agent-saas-pm
console.log(`✅ Found ${result3.length} memories (expected 2 - only unambiguous 3-part IDs)`);
console.log(`   Memory IDs: ${result3.map(m => m.id).join(', ')}`);
if (result3.length !== 2) {
  console.error('❌ FAILED: Expected 2 memories (only unambiguous 3-part IDs)');
